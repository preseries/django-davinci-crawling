...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=None,
        dest="team",
        help="The id of the team where the user belongs" "(ex: --team 450)",
    )
    parser.add_argument(
        "--project-name",
        type=str,
    ...
    parser.add_argument(
        ...
        default="us-east1-b",
        dest="zone",
        help="The GCP zone where provision the cluster" "(ex: --zone us-east1-b)",
    )
    parser.add_argument(
        "--type",
        type=str,
        ...
        dest="cluster_type",
        help="The data science cluster type" "(ex: --type jupiter)",
    )
    parser.add_argument(
        "--master-type",
        type=str,
        ...
        dest="master_type",
        help="The instance type for the spark master" "(ex: --master-type n1-standard-1)",
    )
    parser.add_argument(
        "--worker-type",
        type=str,
        ...
        choices=INSTANCES_TYPE,
        dest="worker_type",
        help="The instance type for the spark workers" "(ex: --worker-type n1-standard-2)",
    )
    parser.add_argument(
        "--worker-instances",
        type=int,
        ...
        help="The number of worker instances" "(ex: --worker-instances 2)",
    )
    ...
    if not cluster_name:
        ...
        if args.cluster_type == "datalab":
            for file_name in ["datalab-cluster-init.sh"]:
                upload_file(storage_client, bucket, "./%s" % file_name, "%s/%s" % (hashed_name, file_name))

            # Cluster initialization script
            init_actions = ["gs://%s/%s/datalab-cluster-init.sh" % (bucket.name, hashed_name)]
            ...
            # Cluster initialization script
            init_actions = ["gs://%s/%s/jupiter-cluster-init.sh" % (bucket.name, hashed_name)]
            ...
            properties = {
                "spark:spark.executorEnv.PYTHONHASHSEED": hash_resource_name("BGDSSeed_201232167312AASSXS22310"),
                "spark:spark.yarn.am.memory": "2048m",
            }
        else:
            raise Exception("Unrecognized cluster type. Valid options are: " "jupiter or datalab")
    ...
    wait_for_cluster(dataproc=dataproc_client, project=args.project_name, region=args.region, cluster_name=cluster_name)

    print("Cluster READY!! you can now open your browser.")

    print(
        "SSH tunneling: [gcloud compute ssh %s-m --project=%s "
        "--zone=%s -- -D 8080 -N -n]" % (cluster_name, args.project_name, args.zone)
    )

    print(
        "Start browser using SOCKS (Mac OS X): "
    ...
    print(
        "Start browser using SOCKS (Linux): "
    ...
    print(
        "Start browser using SOCKS (Windows): "
