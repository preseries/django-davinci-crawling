...
INSTANCES_TYPE = ["n1-standard-1", "n1-standard-2", "n1-standard-4",
...
def upload_file(storage_client, bucket, file_path, file_name):
    ...
    print("Upload file [%s] into [gc://%s/%s]" %
          (file_path, bucket.name, file_name))
...
def create_cluster(dataproc, project, zone, region, cluster_name,
                   ...
                   worker_type="n1-standard-2", worker_instances=2,
                   init_actions=None, metadata=None, properties=None):
    ...
    if init_actions and len(init_actions) > 0:
        cluster_data['config']['initializationActions'] = [
            {"executableFile": file_name for file_name in init_actions}]

    if metadata and len(metadata) > 0:
        cluster_data['config']['gceClusterConfig']['metadata'] = {
...
def submit_pyspark_job(dataproc, project, region, job_details,
                       master_type="n1-standard-1",
    ...
    if 'spark.driver.memory' not in \
            job_details['job']['pysparkJob']['properties']:
        job_details['job']['pysparkJob'][
            'properties']['spark.driver.memory'] = master_memory

    if 'spark.driver.cores' not in \
            job_details['job']['pysparkJob']['properties']:
        job_details['job']['pysparkJob'][
            'properties']['spark.driver.cores'] = master_num_of_cores

    if 'spark.executor.memory' not in \
            job_details['job']['pysparkJob']['properties']:
        job_details['job']['pysparkJob'][
            'properties']['spark.executor.memory'] = worker_memory

    if 'spark.executor.cores' not in \
            job_details['job']['pysparkJob']['properties']:
        job_details['job']['pysparkJob'][
            'properties']['spark.executor.cores'] = worker_num_of_cores
