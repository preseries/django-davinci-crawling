...
class GCPComputeService(object):
    ...
    def commission_instance(
        ...
        machine_type = self._get_config_param(crawler_name, "deployment.machine-type")[0]

        image_project = self._get_config_param(crawler_name, "deployment.image.project")[0]
        image_family = self._get_config_param(crawler_name, "deployment.image.family")[0]

        # Get a reference to the image
        image_response = self.compute.images().getFromFamily(project=image_project, family=image_family).execute()
        ...
        paramaters = ["{} {}".format(key, val) for key, val in crawler_params.items()]

        variables = ["-e {}={}".format(key, val) for key, val in environment_vars.items()]
        ...
        items = [
            ...
            {"key": "crawler-image", "value": crawler_image},
            {"key": "crawler-name", "value": crawler_name},
            {"key": "--current-date", "value": current_date},
            {"key": "--verbosity", "value": verbosity},
            {"key": "--workers-num", "value": workers_num},
            {"key": "--cache-dir", "value": cache_dir},
            {"key": "--local-dir", "value": local_dir},
            {"key": "parameters", "value": " ".join(paramaters)},
            {"key": "environment-vars", "value": " ".join(variables)},
        ]
        ...
        _logger.debug("Entire instance config details: {}".format(json.dumps(config, indent=4)))

        return self.compute.instances().insert(project=project, zone=zone, body=config).execute()

    def decommission_instance(self, crawler_name):
        project = self._get_config_param(crawler_name, "deployment.project")[0]
        zone = self._get_config_param(crawler_name, "deployment.zone")[0]

        return self.compute.instances().delete(project=project, zone=zone, instance=crawler_name).execute()
    ...
    def wait_for_operation(self, crawler_name):
        ...
        while True:
            result = self.compute.zoneOperations().get(project=project, zone=zone, operation=crawler_name).execute()
    ...
    def _get_config_param(cls, crawler_name, param):
        ...
        return (
            jsonpath(crawler_config, param)
            if jsonpath(crawler_config, param)
            else jsonpath(DEFAULT_CONFIG[param], param)
        )
