...
COMPANY_DOCUMENTS_URL = "http://siteempresas.bovespa.com.br/" "consbov/ExibeTodosDocumentosCVM.asp?{}"
...
_logger = logging.getLogger("davinci_crawler_{}.crawling_part.company_files".format(BOVESPA_CRAWLER))
...
def has_files_to_be_processed(ccvm):
    ...
    paginator = (
        CaravaggioSearchPaginator(query_string=str(filter), limit=1, max_limit=1)
    ...
    if paginator.has_next():
        ...
        if paginator.get_hits() > 0:
            _logger.debug("Company {0} HAS {1} FILES PENDING to be processed...".format(ccvm, paginator.get_hits()))
            return True

    _logger.debug("Company {0} has not files pending to be processed...".format(ccvm))
    return False


def extract_ENET_files_from_page(producer, options, ccvm, driver, bs, doc_type, from_date=None, to_date=None):
    """
    ...
    _logger.debug("Extracting files from {0} - {1}".format(company_name, company_cnpj))
    ...
    try:
        ...
        _logger.warning(
            "There is no files information in the companies "
            "files page for [{ccvm} - {doc_type}] ".format(ccvm=ccvm, doc_type=doc_type)
        )
        return files

    while True:
        # Get the number of files we can really get from the current page
        last_file_in_page = int(re.search(RE_LAST_FILE_IN_PAGE, str(bs))[2])
        ...
        # ITR or DFP
        all_tables = [
            tag.findParent("table")
            for tag in bs.find_all(text=re.compile("{} - ENET.*".format(doc_type)))
        ...
        if len(all_tables) > 0:
            for table in all_tables:
                ...
                if link_tag:
                    ...
                    if from_date is not None and (
                        compare_dates(delivery_date, from_date) <= 0 or compare_dates(delivery_date, to_date) >= 0
                    ):
                        continue
                    ...
                    protocol = re.match(RE_DOWNLOAD_FILE, link_tag.attrs["href"])[1]
                    ...
                    try:
                        ...
                        try:
                            file = BovespaCompanyFile.objects.get(
                                ccvm=ccvm, doc_type=doc_type, fiscal_date=fiscal_date, version=version
                            )
                            ...
                            BovespaCompanyFile.create(**company_file_data)
                            producer.add_crawl_params(task_params, options)
                    except LWTException as e:
                        _logger.warning(
                            "The company file [{ccvm} - '{name}' - {doc_type}"
                            " - {fiscal_date:%Y-%m-%d} - {version}]"
                            " cannot be created. The file already exists.".format(
                                ccvm=ccvm,
                                name=company_name,
                                doc_type=doc_type,
                                fiscal_date=fiscal_date,
            ...
            _logger.debug(f"No {doc_type} - ENET files available " f"for company: {ccvm}")
        ...
        if last_file_in_page == num_of_docs:
            ...
            try:
                conditions = [EC.presence_of_element_located((By.XPATH, "//form[@name='AIR']/table/*"))]
                ...
                try:
                    ...
                    _logger.warning(
                        ...
                        "and {doc_type}. Showing 'Error de Aplicacao'".format(ccvm=ccvm, doc_type=doc_type)
                    )
                    raise
...
def obtain_company_files(producer, ccvm, options, doc_type, from_date=None, to_date=None):
    """
    ...
    try:
        ...
        encoded_args = urlencode({"CCVM": ccvm, "TipoDoc": "C", "QtLinks": "1000"})
        ...
        try:
            ...
            try:
                if driver:
                    _logger.debug(
                        "Closing the Selenium Driver for company "
                        "[{ccvm} - {doc_type}]".format(ccvm=ccvm, doc_type=doc_type)
                    )
                    driver.quit()
        ...
        result = driver.find_elements_by_xpath("//form[@name='AIR']/table/*//a")
        if len(result) > 0:
            available_doc_types = [link.text.lower() for link in result if link.text]
            if doc_type.lower() not in available_doc_types:
                _logger.debug(
                    "The OPEN company {ccvm} does not have financial"
                    " reports of {doc_type} type".format(ccvm=ccvm, doc_type=doc_type)
                )
                return ccvm, []
        else:
            _logger.debug(
                "The OPEN company {ccvm} does not have presented"
                " any financial report".format(ccvm=ccvm, doc_type=doc_type)
            )
            return ccvm, []
        ...
        files = extract_ENET_files_from_page(producer, options, ccvm, driver, bs, doc_type, from_date, to_date)
        ...
        result = driver.find_elements_by_xpath(
            "//*[text()='Não há informações disponíveis " "para os parâmetros selecionados.']"
        )
        if len(result) > 0:
            _logger.debug("The company {ccvm} do not have {doc_type} documents".format(ccvm=ccvm, doc_type=doc_type))
            return ccvm, []

        result = driver.find_elements_by_xpath("//*[contains(text(), 'Companhia Aberta não encontrada')]")
        if len(result) > 0:
            _logger.debug("The OPEN company {ccvm} wan not found in Bovespa".format(ccvm=ccvm, doc_type=doc_type))
            return ccvm, []
        ...
        _logger.error(
            f"Unexpected error. The company has documents but there " f"is not list of documents available. URL: {url}"
        )
        return ccvm, None
    except WebDriverException as ex:
        _logger.warning("The company {ccvm} do not have {doc_type} documents".format(ccvm=ccvm, doc_type=doc_type))
        return ccvm, None
    finally:
        _logger.debug(
            "Finishing to crawl company "
            "[{ccvm} - {doc_type}] files: [{num_files}]".format(ccvm=ccvm, doc_type=doc_type, num_files=len(files))
        )
        if driver:
            _logger.debug(
                "Closing the Selenium Driver for company " "[{ccvm} - {doc_type}]".format(ccvm=ccvm, doc_type=doc_type)
            )
            driver.quit()


def crawl_companies_files(options, producer, workers_num=10, include_companies=None, from_date=None, to_date=None):
    """
    ...
    try:
        ...
        ccvm_codes = [
            r.ccvm for r in BovespaCompany.objects.only(["ccvm"]).all() if not has_files_to_be_processed(r.ccvm)
        ]

        ccvm_codes = sorted(ccvm_codes)

        _logger.debug(
            "Processing the files of {0} companies from {1} to {2}".format(
                len(ccvm_codes),
                "{0:%Y-%m-%d}".format(from_date) if from_date else "THE BEGINNING",
                "{0:%Y-%m-%d}".format(to_date) if to_date else "THE END",
            )
        )
        ...
        for ccvm_code in ccvm_codes:
            ...
            for doc_type in DOC_TYPES:
                func_params.append([producer, ccvm_code, options, doc_type, from_date, to_date])
        ...
        return (
            [result for result in call_results if result[1] is not None],
